    ...
    _class("Encrypt", {
        ...
        static_md5 : function(string) {
             ...
             * and its documentation for any purposes and without fee is
            ...
            /*
             * Some basic logical functions had to be rewritten because of a
             * bug in Javascript.. Just try to compute 0xffffffff >> 4 with
             * it.. Of course, these functions are slower than the original
             * would be, but at least, they work!
             */
            ...
            function transform(buf, offset) {
                ...
                for (i = 0; i < 16; i++) {
                    ...
                    for (j = 1; j < 4; j++) {
                        x[i] += shl(and(buf[i * 4 + j + offset], 0xff),
                                j * 8);
                    }
                }
            ...
            function finish() {
                ...
                for (i = 0; i < 4; i++) {
                    for (j = 0; j < 4; j++) {
                        digestBits[i * 4 + j] = and(shr(state[i], (j * 8)),
                                0xff);
                    }
                }
            }
            ...
            for (blockstart = 0; blockstart < word_array.length; blockstart += 16) {
                ...
                for (i = 40; i <= 59; i++) {
                    temp = (rotate_left(A, 5)
                            + ((B & C) | (B & D) | (C & D)) + E + W[i] + 0x8F1BBCDC) & 0x0ffffffff;
                    E = D;
                    D = C;
                    C = rotate_left(B, 30);
                    B = A;
